...
<script>
	...
	function goUP(e){
		if(e==1){
			...
			e=e-1;
			location.href="getActGoods?page="+e+"&pageSize=3&act_id="+$("#act_id").attr("name");
		}
	}
	//下一页
	function goDown(e,f){
		...
		if(pagelast>=pagetotal){
			...
			var page =pagelast+1;
			location.href="getActGoods?page="+page+"&pageSize=3&act_id="+$("#act_id").attr("name");
		}
	}
	//改变图片大消
	function changImg(e){
	...
	function delGoods(e){
		$.ajax({  
		    url: "delActGoods?goods_id="+e.name+"&act_id="+$("#act_id").attr("name"),  
		    ...
		    success: function (responseJSON) {  
		        // your logic
		        alert(responseJSON);
		        location.href="getActGoods?page=1&pageSize=3&act_id="+$("#act_id").attr("name");
		    },
		    error: function (rs){
		...
		<div class="rt_content">
			...
			<section>
		     ...
		     <table class="table">
		      ...
		      <c:forEach items="${ActGoods.data}" var="item">
		      <tr>
		       ...
		       <td>
		        <a href="javascript:void(0)" class="inner_btn" name="${item.id}" onClick="delGoods(this)">解除活动关联</a>
		       </td>
		      </tr>
		      </c:forEach>
		     </table>
		     ...
		     <aside class="paging">
		      <a onClick="goUP(${ActGoods.currentPage})">上一页</a>
		      <a href="#">${ActGoods.currentPage}</a>
		      <a onClick="goDown(${ActGoods.totalPage},${ActGoods.currentPage})">下一页</a>
		     </aside>
